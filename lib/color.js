...
// Secondary is a companion accent, not a second brand color: it sits directly
// opposite the source on the hue circle and carries a little over half its
// chroma, so it reads as the complement of primary while still belonging to
// the same system. The rotation is a half turn because the complementary
// relationship is the requirement; the chroma scale is still the kit's number
// (see docs/tokens/figma-snapshot.json, `Graphite Primitives/secondary/*`),
// which was built around a −120° turn — so the ramp no longer reproduces the
// snapshot's secondary hexes. That divergence is intended, not drift.
const SECONDARY_HUE_TURN = 180 * (Math.PI / 180)
...
export function makeRamps(hex) {
  ...
  // because the kit samples it there rather than at a round 50. That is a
  // structural choice about where the ramp is sampled, independent of which
  // way it is rotated, so it survives the move to a complementary hue.
  //
  ...
  // is the opposite hue, and marking it would make the UI label the complement
...
export const STATE_FAMILIES = ['primary', 'secondary']
...
export function buildStates(tokens, ramps, mode) {
  const primary = familyStates(tokens.primary, ramps, mode)
  const secondary = familyStates(tokens.secondary, ramps, mode)
  ...
  // reads; it is primary's ring, not a third value.
  return { primary, secondary, focus: primary.focus }
}
