...
function hexToOklab(hex) {
  ...
  const linear = { r: srgbToLinear(rgb.r), g: srgbToLinear(rgb.g), b: srgbToLinear(rgb.b) }
...
function maxChromaInGamut(L, hueRad, upperBound) {
  const linearAt = (c) => oklabToLinearRgb({ L, a: c * Math.cos(hueRad), b: c * Math.sin(hueRad) })
...
// Returns { accent, neutral, neutralVariant }, each { tone(fn), stops, sourceTone? }.
// tone() is a continuous function so callers can resolve any tone, not just the
// named stops. The source hex is pinned into the accent ramp at its true tone.
export function makeRamps(hex) {
  ...
  // the three above are — their hue is fixed — so anything presenting the
  // source-derived ramps should keep listing those three explicitly.
  ...
  for (const name of STATUS_NAMES) {
    ...
    statusRamps[name] = { tone: fn, stops: buildRampStops(fn, sourceTone, false) }
  }

  return {
    accent: { tone: accentFn, stops: accentStops, sourceTone },
    neutral: { tone: neutralFn, stops: buildRampStops(neutralFn, sourceTone, false) },
    neutralVariant: { tone: neutralVariantFn, stops: buildRampStops(neutralVariantFn, sourceTone, false) },
    ...statusRamps,
  }
}
...
// Status has to stay recognizable: red must read as "error" whatever the
...
// would otherwise produce a gray "error" and a neon one a garish red.
//
...
export const STATUS_NAMES = ['error', 'warning', 'success', 'info']
...
function pairsFor(textTarget, uiTarget) {
  ...
  return {
    ...
    onPrimaryContainer: { base: 'primaryContainer', target: textTarget, kind: 'AA' },
    onSurface: { base: 'surface', target: textTarget, kind: 'AA' },
    onSurfaceVariant: { base: 'surfaceVariant', target: textTarget, kind: 'AA' },
...
export function buildTheme(mode, ramps, level = 'AA', autoFix = true) {
  ...
  for (const [role, ref] of Object.entries(map)) {
    tokens[role] = { ramp: ref.ramp, tone: ref.tone, hex: resolveTone(ramps, ref.ramp, ref.tone) }
  }
...
// dark mode. Focus is a separate ring token from the accent ramp, not a fill
// change on the base token.
export function buildStates(tokens, ramps, mode) {
  const direction = mode === 'dark' ? 1 : -1
  const baseRole = tokens.primary
  const rampFn = ramps[baseRole.ramp].tone

  const primary = {
    base: { ramp: baseRole.ramp, tone: baseRole.tone, hex: baseRole.hex },
  }
  for (const [state, delta] of Object.entries(STATE_DELTAS)) {
    const tone = clamp(baseRole.tone + direction * delta, 0, 100)
    primary[state] = { ramp: baseRole.ramp, tone, hex: rampFn(tone) }
  }
  ...
  const focus = { ramp: 'accent', tone: focusTone, hex: ramps.accent.tone(focusTone) }
...
function themeBundle(bundle) {
  ...
  return [
    { selector: ':root, [data-theme="light"]', theme: light, states: lightStates },
...
export function buildCss(bundle) {
  ...
  for (const { selector, theme, states } of themeBundle(bundle)) {
    ...
    for (const [state, entry] of Object.entries(states.primary)) {
      const name = state === 'base' ? 'primary' : `primary-${state}`
      ...
      if (entry.content) {
        ...
        lines.push(`  --cts-${name}-content-tone: ${round1(entry.content.tone)};`)
      }
    }
...
function themeToJson(theme, states) {
  return {
    tokens: Object.fromEntries(Object.entries(theme.tokens).map(([role, t]) => [role, tokenEntry(t)])),
    contrast: theme.contrast,
    states: {
      primary: Object.fromEntries(Object.entries(states.primary).map(([s, e]) => [s, stateEntry(e)])),
...
export function buildJson(bundle) {
  ...
  const rampToJson = (ramp) => ramp.stops.map((s) => ({ tone: s.tone, value: s.hex, source: !!s.source }))
