...
- *Carbon idioms with no Graphite counterpart* — Structured list, Contained
  list, Toggletip, Tile, Code snippet, Loading, Progress indicator.
...
Input; Dropdown is Select, or Dropdown Menu for the menu case. These are
...
No component contract carries a **[blocked on Wave 0]** marker any more. The two Wave 5 dependencies that used to be listed here as open — the shared overlay surface token and the Dialog scrim — both closed in #92: `surfaceElevated` exists in the kit's `Graphite Semantic` collection, and `scrim` is generated by `scrimFor()` in `lib/color.js` rather than declared as a literal. The drift check reports no warnings for either.
